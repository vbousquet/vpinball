...
$outputs = @('metal','essl','glsl','dx9','dx11', 'spirv')
...
for($i = 0; $i -lt 6; $i++)
{
   ...
   foreach ($variant in $variants)
   {
      ...
      {
		   $Parms = ("-f fs_basic.sc -o " + $outputs[$i] + "/fs_basic_" + $variant.ToLower() + "_" + $subvariant.ToLower() + ".bin --platform " + $platforms[$i] + " -p " + $fprofiles[$i] + " --type fragment --define " + $variant + ";" + $subvariant).Split(" ")
		   & "$shaderc" $Parms
	  }
   }
   ...
   foreach ($variant in $variants)
   {
	   $Parms = ("-f vs_dmd.sc -o " + $outputs[$i] + "/vs_basic_dmd_" + $variant.ToLower() + ".bin --platform " + $platforms[$i] + " -p " + $vprofiles[$i] + " --type vertex --define " + $variant).Split(" ")
	   & "$shaderc" $Parms
   }
   $variants = @("DMD", "SPRITE")
   foreach ($variant in $variants)
   {
	   $Parms = ("-f fs_dmd.sc -o " + $outputs[$i] + "/fs_basic_" + $variant.ToLower() + ".bin --platform " + $platforms[$i] + " -p " + $fprofiles[$i] + " --type fragment --define " + $variant).Split(" ")
	   & "$shaderc" $Parms
   }
   ...
   foreach ($variant in $variants)
   {
	   ...
	   {
		   $Parms = ("-f vs_classic_light.sc -o " + $outputs[$i] + "/vs_classic_light_" + $variant2.ToLower() + "_" + $variant.ToLower() + ".bin --platform " + $platforms[$i] + " -p " + $vprofiles[$i] + " --type vertex --define " + $variant + ";" + $variant2).Split(" ")
		   & "$shaderc" $Parms
		   $Parms = ("-f fs_classic_light.sc -o " + $outputs[$i] + "/fs_classic_light_" + $variant2.ToLower() + "_" + $variant.ToLower() + ".bin --platform " + $platforms[$i] + " -p " + $fprofiles[$i] + " --type fragment --define " + $variant + ";" + $variant2).Split(" ")
		   & "$shaderc" $Parms
	   }
   }
   ...
   $Parms = ("-f vs_postprocess_offseted.sc -o " + $outputs[$i] + "/vs_postprocess_offseted.bin --platform " + $platforms[$i] + " -p " + $vprofiles[$i] + " --type vertex").Split(" ")
   & "$shaderc" $Parms
   ...
   $Parms = ("-f fs_pp_bloom.sc -o " + $outputs[$i] + "/fs_pp_bloom.bin --platform " + $platforms[$i] + " -p " + $fprofiles[$i] + " --type fragment").Split(" ")
   & "$shaderc" $Parms

   $Parms = ("-f fs_pp_ssao.sc -o " + $outputs[$i] + "/fs_pp_ssao.bin --platform " + $platforms[$i] + " -p " + $fprofiles[$i] + " --type fragment").Split(" ")
   & "$shaderc" $Parms

   $Parms = ("-f fs_pp_ssr.sc -o " + $outputs[$i] + "/fs_pp_ssr.bin --platform " + $platforms[$i] + " -p " + $fprofiles[$i] + " --type fragment").Split(" ")
   & "$shaderc" $Parms

   $Parms = ("-f fs_pp_cas.sc -o " + $outputs[$i] + "/fs_pp_cas.bin --platform " + $platforms[$i] + " -p " + $fprofiles[$i] + " --type fragment").Split(" ")
   & "$shaderc" $Parms

   $Parms = ("-f fs_pp_bilateral_cas.sc -o " + $outputs[$i] + "/fs_pp_bilateral_cas.bin --platform " + $platforms[$i] + " -p " + $fprofiles[$i] + " --type fragment").Split(" ")
   & "$shaderc" $Parms

   $Parms = ("-f fs_pp_nfaa.sc -o " + $outputs[$i] + "/fs_pp_nfaa.bin --platform " + $platforms[$i] + " -p " + $fprofiles[$i] + " --type fragment").Split(" ")
   & "$shaderc" $Parms

   $variants = @("FXAA1", "FXAA2", "FXAA3")
   foreach ($variant in $variants)
   {
	   $Parms = ("-f fs_pp_fxaa.sc -o " + $outputs[$i] + "/fs_pp_" + $variant.ToLower() + ".bin --platform " + $platforms[$i] + " -p " + $fprofiles[$i] + " --type fragment --define " + $variant).Split(" ")
	   & "$shaderc" $Parms
   }
