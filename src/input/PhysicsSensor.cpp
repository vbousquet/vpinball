...
void PhysicsSensor::SetMapping(const SensorMapping& mapping)
{
   ...
   if (mapping.GetType() == SensorMapping::Type::Position && m_sensorType == SensorMapping::Type::Acceleration)
      m_integrator = std::make_unique<BasicSecondDerivative>();
      ...
      m_integrator = std::make_unique<BasicFirstDerivative>();
   else if (mapping.GetType() == SensorMapping::Type::Acceleration && m_sensorType == SensorMapping::Type::Position)
   {
      ...
      assert(false);
      m_integrator = std::make_unique<NoOpInputFilter>();
   }
...
void PhysicsSensor::OnInputChanged(SensorMapping* mapping)
{
   ...
   // The integrator is based on the data time stamp, therefore 'Get' delivers a stable value until next 'Push'
   m_integrator->Push(mapping->GetRawValueTimestampNs(), mapping->GetValue());

   // The filter is based on the game time (evalued in Get, not Push), not the data timestamp
   m_filter->Push(mapping->GetRawValueTimestampNs(), m_integrator->Get());
}
...
void PhysicsSensor::Override(float value)
{
   ...
   m_filter->Push(0, m_integrator->Get());
}
