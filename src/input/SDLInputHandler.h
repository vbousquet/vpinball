...
private:
   string GetJoySettingId(SDL_Joystick* joy) const
   {
      ...
      if (crc16 == 0)
      {
         PLOGE << "Joystick '" << SDL_GetJoystickName(joy) << "' does not provide a serial number nor a device CRC, using GUID instead. This may results in input conflicts.";
      }
      else
      {
         PLOGD << "Joystick '" << SDL_GetJoystickName(joy) << "' does not provide a serial number but a GUID with a device CRC instead. This may eventually results in input conflicts.";
      }
      char strGuid[33];
   ...
   void OnJoystickAdded(SDL_JoystickID id)
   {
      ...
      else
      {
         ...
         {
            std::stringstream ss;
            ss << SDL_GetJoystickName(joystick) << " #" << std::hex << std::setw(4) << std::setfill('0') << crc16;
            joyName = ss.str();
         }
         else
         {
      ...
      // we are interested on the button binding for better button naming (if available) and for broader gamepad layout detection
      if (SDL_IsGamepad(id))
      {
            ...
            if (SDL_GamepadBinding** bindings = SDL_GetGamepadBindings(gamepad, &nBindings); bindings)
            {
               for (int i = 0; i < nBindings; i++)
               {
                  ...
                  else if (bindings[i]->input_type == SDL_GAMEPAD_BINDTYPE_AXIS && bindings[i]->output_type == SDL_GAMEPAD_BINDTYPE_AXIS)
                  {
                     ...
                     if (!name.empty())
                     {
                        registeredAxisMask |= 1ULL << bindings[i]->input.axis.axis;
                  ...
                  && southButton != 0xFFFF && backButton != 0xFFFF && startButton != 0xFFFF && leftStickX != 0xFFFF && leftStickY != 0xFFFF && rightStickY != 0xFFFF)
               {
                     ...
                     std::function<bool(const SensorMapping&, const SensorMapping&, SensorMapping::Type type)> mapNudge)
                  {
                     ...
                     success &= mapNudge(SensorMapping::Create(deviceId, leftStickX, SensorMapping::Type::Position), SensorMapping::Create(deviceId, leftStickY, SensorMapping::Type::Position), SensorMapping::Type::Position);
                     return success;
                  };
         ...
         // Also register the axis that are not recognized & mapped to a standard gamepad axis to allow selection in UI
         for (int axis = 0; axis < nAxis; axis++)
            if ((registeredAxisMask & (1ULL << axis)) == 0)
