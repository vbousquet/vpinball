...
void BasicFirstDerivative::Push(uint64_t timestampNs, float value)
{
   if (!m_initialized)
   {
      m_initialized = true;
   ...
   constexpr float timeFactor = 1000000000.f * DEFAULT_STEPTIME_S; // Nanoseconds to Visual Pinball Time
   const float dt1 = timeFactor / static_cast<float>(m_t0 - m_t1);

   m_value = (m_p0 - m_p1) * dt1;

   // To avoid VBS overflows
   m_value = clamp(m_value, -1000.f, 1000.f);

   /* PLOGD << "------------------- ";
   PLOGD << m_t1 << " = " << m_p1 << " ==> " << m_value;
   PLOGD << m_t0 << " = " << m_p0 << " ==> " << m_value;*/
}


void BasicSecondDerivative::Push(uint64_t timestampNs, float value)
{
   if (!m_initialized)
   {
      m_initialized = true;
      m_t2 = timestampNs - 32000000;
      m_t1 = timestampNs - 16000000;
      m_t0 = timestampNs;
      m_p2 = m_p1 = m_p0 = value;
   }
   else if (timestampNs == m_t0)
   {
      m_p0 = value;
   ...
   constexpr float timeFactor = 1000000000.f * DEFAULT_STEPTIME_S; // Nanoseconds to Visual Pinball Time
   const float dt1 = timeFactor / static_cast<float>(m_t0 - m_t1);
   const float dt2 = timeFactor / static_cast<float>(m_t1 - m_t2);

   // Compute velocities (first derivative)
   const float v1 = (m_p1 - m_p2) * dt2;
   const float v0 = (m_p0 - m_p1) * dt1;
   ...
   /* PLOGD << "------------------- ";
   PLOGD << m_t2 << " = " << m_p2 << " ==> " << m_value;
   PLOGD << m_t1 << " = " << m_p1 << " ==> " << m_value;
   PLOGD << m_t0 << " = " << m_p0 << " ==> " << m_value;*/
}


void PlungerPositionFilter::AdvanceFilter()
{
   // If we lost sync (defined at 30ms as we are supposed to be synced for every ms step), just resync
   if (m_filterTimestampMs + 30 < g_pplayer->m_time_msec)
      m_filterTimestampMs = g_pplayer->m_time_msec;

   // Advance the filter by steps of 1ms until up to date with game (1KHz sampling, aligned on physics engine)
   while (m_filterTimestampMs < g_pplayer->m_time_msec)
   {
      m_x[0] = m_sensorValue;
...
void NudgeAccelerationFilter::AdvanceFilter()
{
   // If we lost sync (defined at 30ms as we are supposed to be synced for every ms step), just resync
   if (m_filterTimestampMs + 30 < g_pplayer->m_time_msec)
      m_filterTimestampMs = g_pplayer->m_time_msec;
