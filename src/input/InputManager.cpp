...
void InputManager::RegisterDefaultMapping(uint16_t deviceId,
   const std::function<bool( // Function to either apply or evaluate applying the default mapping of the given controller
      ...
      std::function<bool(const SensorMapping&, const SensorMapping&, SensorMapping::Type)> // Map nudge
      )>& mapper)
{
   assert(deviceId < m_inputDevices.size());
   ...
   {
      const std::unique_ptr<PhysicsSensor>& plungerPos = GetPlungerPositionSensor();
      return plungerPos->IsMapped() && plungerPos->GetMapping().IsSame(sensorPos);
   };
   auto isNudgeMapped = [this](const SensorMapping& sensorX, const SensorMapping& sensorY, SensorMapping::Type type)
   {
      // TODO always map to first board ? or add to the first free slot ?
      const std::unique_ptr<PhysicsSensor>& nudgeX = GetNudgeXSensor(0);
      const std::unique_ptr<PhysicsSensor>& nudgeY = GetNudgeYSensor(0);
      return nudgeX->IsMapped() && nudgeX->GetMapping().IsSame(sensorX) && nudgeY->IsMapped() && nudgeY->GetMapping().IsSame(sensorY);
   };
...
void InputManager::ApplyDefaultDeviceMapping(uint16_t deviceId)
{
   ...
   auto mapNudge = [this](const SensorMapping& sensorX, const SensorMapping& sensorY, SensorMapping::Type type)
   {
      // TODO always map to first board ? or add to the first free slot ?
      const std::unique_ptr<PhysicsSensor>& nudgeX = GetNudgeXSensor(0);
      const std::unique_ptr<PhysicsSensor>& nudgeY = GetNudgeYSensor(0);
...
void InputManager::ProcessInput()
{
   ...
   if (m_hasPendingLayoutApply && g_pplayer->m_liveUI)
   {
      ...
      for (auto& device : m_inputDevices)
      {
         ...
         {
            if (g_pvp->m_settings.LoadValueWithDefault(Settings::Input, "Device." + device.m_settingsId + ".DisableLayout", false))
            {
            ...
            if (g_pplayer->m_liveUI->ProposeInputLayout(device.m_name,
                   [this, deviceId](bool isOk, bool isDontAskAnymore)
                   {
                      ...
                      if (isDontAskAnymore)
                         g_pvp->m_settings.SaveValue(Settings::Input, "Device." + m_inputDevices[deviceId].m_settingsId + ".DisableLayout", true);
                   }))
            {
               device.m_hasPendingLayoutApply = false;
...
void InputManager::SetNudgeFiltered(int index, bool enable)
{
   ...
   {
      m_nudgeXSensor[index]->SetFilter(std::make_unique<NoOpInputFilter>());
      m_nudgeYSensor[index]->SetFilter(std::make_unique<NoOpInputFilter>());
   }
}
