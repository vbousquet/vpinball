...
public:
   ...
   void HitTestBall(const HitBall* const pball, CollisionEvent& coll) const { m_rootNode.HitTestBall(pball, coll); }
   void HitTestXRay(const HitBall* const pball, vector<HitTestResult>& pvhoHit, CollisionEvent& coll) const { m_rootNode.HitTestXRay(pball, pvhoHit, coll); }
...
private:
   ...
   unsigned int m_num_items; // alias of m_vho.size()
