...
void PlungerSettingsPage::Render()
{
   ...
   if (m_positionPlot.HasData() && ImPlot::BeginPlot("##Script", ImVec2(-1, -1), ImPlotFlags_None))
   {
      ...
      if (m_player->m_pininput.HasMechPlungerSpeed())
      {
         ...
         ImPlot::PlotLine("Speed", &m_velocityPlot.m_data[0].x, &m_velocityPlot.m_data[0].y, m_velocityPlot.m_data.size(), 0, m_velocityPlot.m_offset, 2 * sizeof(float));
         ImPlot::PopStyleColor();
      }
      ImPlot::PlotLine("Position", &m_positionPlot.m_data[0].x, &m_positionPlot.m_data[0].y, m_positionPlot.m_data.size(), 0, m_positionPlot.m_offset, 2 * sizeof(float));
      ImPlot::EndPlot();
   }
