...
void InGameUIPage::Open() { m_selectedItem = 0; }
...
void InGameUIPage::SelectNextItem()
{
   ...
   while (!m_items[m_selectedItem]->IsInteractive());
}
...
void InGameUIPage::Render()
{
   ...
   for (int i = 0; i < m_items.size(); i++)
   {
      ...
      const bool hoovered
         = (m_player->m_liveUI->m_inGameUI.IsFlipperNav() && i == m_selectedItem) || (!m_player->m_liveUI->m_inGameUI.IsFlipperNav() && isMouseOver && item->IsInteractive());
      if (hoovered)
      {
