...
void PointOfViewSettingPage::Close()
{
   assert(m_opened);
   m_opened = false;
...
void PointOfViewSettingPage::Save()
{
...
void PointOfViewSettingPage::OnPointOfViewChanged()
{
   assert(m_opened);
   if (!m_staticPrepassDisabled)
   {
...
void PointOfViewSettingPage::BuildPage()
{
   assert(m_opened);
   const Settings& settings = GetSettings();
   PinTable* table = m_player->m_ptable;
   ...
   auto viewMode = std::make_unique<InGameUIItem>(InGameUIItem { "View mode"s,
      ...
      { "Legacy"s, "Camera"s, "Window"s }, static_cast<int>(viewSetup.mMode), static_cast<int>(defViewSetup.mMode),
      [this](int v)
      {
         PinTable* table = m_player->m_ptable;
         table->mViewSetups[table->m_BG_current_set].mMode = static_cast<ViewLayoutMode>(v);
   ...
   auto layback = std::make_unique<InGameUIItem>(
      "Layback"s, "Fake visual stretch of the table to give more depth"s, 0.f, 90.f, 0.1f, viewSetup.mLayback, defViewSetup.mLayback, "%4.1f  "s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
         viewSetup.mLayback = v;
         OnPointOfViewChanged();
      },
   ...
   auto fov = std::make_unique<InGameUIItem>("Field Of View (overall scale)"s, "Global view scale (same as XYZ scale)"s, 25.f, 90.f, 0.1f, viewSetup.mFOV, defViewSetup.mFOV, "%4.1f °"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
   ...
   auto xScale = std::make_unique<InGameUIItem>("Table X Scale"s, "Stretch the scene along the playfield width axis"s, 50.f, 150.f, 0.1f, 100.f * viewSetup.mSceneScaleX / realToVirtual,
      100.f * defViewSetup.mSceneScaleX / defViewSetup.GetRealToVirtualScale(table), "%4.1f %%"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
         viewSetup.mSceneScaleX = v * viewSetup.GetRealToVirtualScale(table) / 100.f;
         OnPointOfViewChanged();
      },
   ...
   auto yScale = std::make_unique<InGameUIItem>("Table Y Scale"s, "Stretch the scene along the playfield height axis"s, 50.f, 150.f, 0.1f, 100.f * viewSetup.mSceneScaleY / realToVirtual,
      100.f * defViewSetup.mSceneScaleY / defViewSetup.GetRealToVirtualScale(table), "%4.1f %%"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
         viewSetup.mSceneScaleY = v * viewSetup.GetRealToVirtualScale(table) / 100.f;
         OnPointOfViewChanged();
      },
   ...
   auto zScale = std::make_unique<InGameUIItem>("Table Z Scale"s, "Stretch the scene along the vertical axis (perpendicular to playfield)"s, 50.f, 150.f, 0.1f,
      100.f * viewSetup.mSceneScaleZ / realToVirtual, 100.f * defViewSetup.mSceneScaleZ / defViewSetup.GetRealToVirtualScale(table), "%4.1f %%"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
         viewSetup.mSceneScaleZ = v * viewSetup.GetRealToVirtualScale(table) / 100.f;
         OnPointOfViewChanged();
      },
   ...
   auto playerX = std::make_unique<InGameUIItem>("Player X", "Player position in real world, expressed from the bottom center of the playfield, in centimeters"s, -30.f, 30.f, 0.1f,
      m_playerPos.x, m_initialPlayerPos.x, "%4.1f cm"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
   ...
   auto playerY = std::make_unique<InGameUIItem>("Player Y", "Player position in real world, expressed from the bottom center of the playfield, in centimeters"s, -70.f, 30.f, 0.1f,
      m_playerPos.y, m_initialPlayerPos.y, "%4.1f cm"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
   ...
   auto playerZ = std::make_unique<InGameUIItem>("Player Z", "Player position in real world, expressed from the bottom center of the playfield, in centimeters"s, 30.f, 100.f, 0.1f,
      m_playerPos.z, m_initialPlayerPos.z, "%4.1f cm"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
   ...
   auto viewX = std::make_unique<InGameUIItem>(isLegacy ? "X Offset" : "Camera X", "View point width offset, in centimeters"s, -30.f, 30.f, 0.1f, VPUTOCM(viewSetup.mViewX),
      VPUTOCM(defViewSetup.mViewX), "%4.1f cm"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
         viewSetup.mViewX = CMTOVPU(v);
         OnPointOfViewChanged();
      },
   ...
   auto viewY = std::make_unique<InGameUIItem>(isLegacy ? "Y Offset" : "Camera Y", "View point height offset, in centimeters"s, -30.f, 100.f, 0.1f, VPUTOCM(viewSetup.mViewY),
      VPUTOCM(defViewSetup.mViewY), "%4.1f cm"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
         viewSetup.mViewY = CMTOVPU(v);
         OnPointOfViewChanged();
      },
   ...
   auto viewZ = std::make_unique<InGameUIItem>(isLegacy ? "Z Offset" : "Camera Z", "View point vertical offset, in centimeters"s, 10.f, 100.f, 0.1f, VPUTOCM(viewSetup.mViewZ),
      VPUTOCM(defViewSetup.mViewZ), "%4.1f cm"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
         viewSetup.mViewZ = CMTOVPU(v);
         OnPointOfViewChanged();
      },
   ...
   auto hOfs = std::make_unique<InGameUIItem>("Horizontal Offset", "Horizontal offset of the virtual table behind the screen 'window' in centimeters"s, -30.f, 30.f, 0.1f,
      viewSetup.mViewHOfs, defViewSetup.mViewHOfs, "%4.1f cm"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
         viewSetup.mViewHOfs = v;
         OnPointOfViewChanged();
      },
   ...
   auto vOfs = std::make_unique<InGameUIItem>("Vertical Offset", "Vertical offset of the virtual table behind the screen 'window' in centimeters"s, -30.f, 30.f, 0.1f, viewSetup.mViewVOfs,
      defViewSetup.mViewVOfs, "%4.1f cm"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
         viewSetup.mViewVOfs = v;
         OnPointOfViewChanged();
      },
   ...
   auto wndTopZ = std::make_unique<InGameUIItem>("Window Top Z Ofs.", "Distance between the 'window' (i.e. the screen) at the top of the playfield, in centimeters"s, 0.f, 50.f, 0.1f,
      VPUTOCM(viewSetup.mWindowTopZOfs), VPUTOCM(defViewSetup.mWindowTopZOfs), "%4.1f cm"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
         viewSetup.mWindowTopZOfs = CMTOVPU(v);
         OnPointOfViewChanged();
      },
   ...
   auto wndBotZ = std::make_unique<InGameUIItem>(
      "Window Bottom Z Ofs.", "Distance between the 'window' (i.e. the screen) at the bottom of the playfield, in centimeters"s, 0.f, 50.f, 0.1f, VPUTOCM(viewSetup.mWindowBottomZOfs),
      VPUTOCM(defViewSetup.mWindowBottomZOfs), "%4.1f cm"s,
      [this](float v)
      {
         PinTable* table = m_player->m_ptable;
         ViewSetup& viewSetup = table->mViewSetups[table->m_BG_current_set];
         viewSetup.mWindowBottomZOfs = CMTOVPU(v);
         OnPointOfViewChanged();
      },
