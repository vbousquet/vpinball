...
#define PSC_CLASS_START(name) \
   ...
   template<typename F> void Register##name##SCD(F&& regFunc) { \
      ...
      members.push_back( { { "AddRef" }, { "ulong" }, 0, { }, \
         [](void* me, int, ScriptVariant* pArgs, ScriptVariant* pRet) { long rc = static_cast<name *>(me)->AddRef(); if (pRet != nullptr) pRet->vLong = rc; } } ); \
      members.push_back( { { "Release" }, { "ulong" }, 0, { }, \
         [](void* me, int, ScriptVariant* pArgs, ScriptVariant* pRet) { long rc = static_cast<name *>(me)->Release(); if (pRet != nullptr) pRet->vLong = rc; } } );
