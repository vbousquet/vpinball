...
void Renderer::RenderDMD(const int w, const int h, BaseTexture* dmd, RenderTarget* rt)
{
   ...
   const vec4 dotColor(1.f, 0.8f, 0.6f, 0.f);
   SetupDMDRender(dotColor, dmd, 1.f, true);
...
void Renderer::SetupDMDRender(const vec4& color, BaseTexture* dmd, const float alpha, const bool sRGB)
{
   ...
   */
   ...
   const float dotSize = 0.85f;
   const float dotSharpness = 0.8f;
   const float dotRounding = 0.85f;
   const float dotGlow = 0.3f;
   const float backGlow = 0.4f;
   const vec4 dotColor(1.f, 0.8f, 0.6f, 0.f);
   const vec4 unlitDotColor(0.0125f, 0.0125f, 0.0125f, 0.f);
   ...
   m_renderDevice->m_DMDShader->SetVector(SHADER_w_h_height, dotSize, dotSharpness, dotRounding, 0.f /* unused */);
   ...
   m_renderDevice->m_DMDShader->SetVector(SHADER_staticColor_Alpha, unlitDotColor.x, unlitDotColor.y, unlitDotColor.z, 0.f /* unused */);
   m_renderDevice->m_DMDShader->SetVector(SHADER_vRes_Alpha_time, (float)dmd->width(), (float)dmd->height(), dotGlow * brightness, backGlow * brightness);
