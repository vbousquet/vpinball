...
PUPLabel::PUPLabel(PUPManager* manager, const string& szName, const string& szFont, float size, int color, float angle, PUP_LABEL_XALIGN xAlign, PUP_LABEL_YALIGN yAlign, float xPos, float yPos, int pagenum, bool visible)
   : m_pManager(manager)
{
   m_szName = szName;
   ...
   if (m_dirty) {
      if (!m_szPath.empty()) {
         ...
         if (m_type == PUP_LABEL_TYPE_IMAGE) {
            // FIXME port m_pTexture = IMG_LoadTexture(pRenderer, m_szPath.c_str());
            m_pTexture = nullptr;
            if (m_pTexture)
               m_dirty = false;
            else {
...
void PUPLabel::UpdateLabelTexture(SDL_Rect& rect)
{
   ...
   if (!pTextSurface) {
      ...
      if (warnedLabels[szError].find(m_szName) == warnedLabels[szError].end()) {
         LOGE("Unable to render text: label=%s, error=%s", m_szName.c_str(), szError.c_str());
