...
void onGetSegSrc(const unsigned int eventId, void* userData, void* msgData)
{
   ...
   {
      msg.entries[msg.count].id = { endpointId, i };
...
void onGetRenderDMDSrc(const unsigned int eventId, void* userData, void* msgData)
{
   ...
   for (unsigned int index = 0; index < state->nDisplays; index++)
   {
      ...
      {
         msg.entries[msg.count].id = { endpointId, frame->displayId };
...
MSGPI_EXPORT void MSGPIAPI PluginLoad(const unsigned int sessionId, MsgPluginAPI* api)
{
   ...
   Controller_SCD->CreateObject = []()
   {
      ...
      if (vpxApi != nullptr)
      {
         ...
         pinmamePath = find_directory_case_insensitive(tableInfo.path, "pinmame");
      }
