...
/* void B2SReelBox::OnPaint(VP::RendererGraphics* pGraphics)
{
   if (IsVisible()) {
      if (!m_szReelIndex.empty()) {
         GenericDictionaryIgnoreCase<Texture*>* pImages = (m_illuminated ? m_pB2SData->GetReelIlluImages() : m_pB2SData->GetReelImages());
         GenericDictionaryIgnoreCase<Texture*>* pIntImages = (m_illuminated ? m_pB2SData->GetReelIntermediateIlluImages() : m_pB2SData->GetReelIntermediateImages());
         string name;
