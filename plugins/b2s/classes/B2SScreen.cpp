...
void B2SScreen::ReadB2SSettingsFromFile()
{
   ...
   m_backglassSize = { 0, 0, pSettings->LoadValueWithDefault(Settings::Standalone, "B2SBackglassWidth"s, B2S_SETTINGS_BACKGLASSWIDTH),
      pSettings->LoadValueWithDefault(Settings::Standalone, "B2SBackglassHeight"s, B2S_SETTINGS_BACKGLASSHEIGHT) };

   m_backglassLocation = { pSettings->LoadValueWithDefault(Settings::Standalone, "B2SBackglassX"s, B2S_SETTINGS_BACKGLASSX),
      pSettings->LoadValueWithDefault(Settings::Standalone, "B2SBackglassY"s, B2S_SETTINGS_BACKGLASSY) };

   m_dmdSize = { 0, 0, pSettings->LoadValueWithDefault(Settings::Standalone, "B2SDMDWidth"s, B2S_SETTINGS_DMDWIDTH),
      pSettings->LoadValueWithDefault(Settings::Standalone, "B2SDMDHeight"s, B2S_SETTINGS_DMDHEIGHT) };

   m_dmdLocation
      = { pSettings->LoadValueWithDefault(Settings::Standalone, "B2SDMDX"s, B2S_SETTINGS_DMDX), pSettings->LoadValueWithDefault(Settings::Standalone, "B2SDMDY"s, B2S_SETTINGS_DMDY) };
...
void B2SScreen::GetB2SSettings(SDL_Point defaultDMDLocation, eDMDViewMode dmdViewMode, int backglassGrillHeight, int backglassSmallGrillHeight)
{
   ...
   if (showTheGrill)
   {
      ...
      if (backglassSmallGrillHeight > 0 && pFormBackglass->GetDarkImage())
      {
         ...
         m_backglassCutOff = { 0, (int)pFormBackglass->GetDarkImage()->m_height - backglassGrillHeight - backglassSmallGrillHeight, (int)pFormBackglass->GetDarkImage()->m_width,
            backglassSmallGrillHeight };
         ...
         pFormBackglass->SetSize({ 0, 0, (int)pFormBackglass->GetDarkImage()->m_width, (int)pFormBackglass->GetDarkImage()->m_height });
      }
   }
   else
   {
      ...
      if (backglassGrillHeight > 0 && pFormBackglass->GetDarkImage())
      {
         ...
         m_backglassCutOff = { 0, (int)pFormBackglass->GetDarkImage()->m_height - backglassGrillHeight, (int)pFormBackglass->GetDarkImage()->m_width, backglassGrillHeight };
...
void B2SScreen::Show()
{
   ...
   m_dmdToBeShown = (m_pFormDMD && !SDL_RectEmpty(&m_dmdSize) && !(m_dmdLocation.x == 0 && m_dmdLocation.y == 0)
      && ((m_dmdViewMode == eDMDViewMode_ShowDMD) || (m_dmdViewMode == eDMDViewMode_ShowDMDOnlyAtDefaultLocation && m_dmdAtDefaultLocation)
   ...
   if (m_pFormBackglass && m_pFormBackglass->GetBackgroundImage())
      m_pFormBackglass->SetSize({ 0, 0, (int)m_pFormBackglass->GetBackgroundImage()->m_width, (int)m_pFormBackglass->GetBackgroundImage()->m_height });
   if (m_pFormDMD && m_pFormDMD->GetBackgroundImage())
      m_pFormDMD->SetSize({ 0, 0, (int)m_pFormDMD->GetBackgroundImage()->m_width, (int)m_pFormDMD->GetBackgroundImage()->m_height });
   ...
   {
      rescaleBackglassX = (float)m_pFormBackglass->GetBackgroundImage()->m_width / (float)m_backglassSize.w;
      rescaleBackglassY = (float)m_pFormBackglass->GetBackgroundImage()->m_height / (float)m_backglassSize.h;
   }
   ...
   if (m_dmdToBeShown)
   {
      ...
      {
         rescaleDMDX = (float)m_pFormDMD->GetBackgroundImage()->m_width / (float)m_dmdSize.w;
         rescaleDMDY = (float)m_pFormDMD->GetBackgroundImage()->m_height / (float)m_dmdSize.h;
      }
   }
...
void B2SScreen::ScaleAllControls(float rescaleX, float rescaleY, float rescaleDMDX, float rescaleDMDY)
{
   ...
   for (const auto& [key, pLedArea] : *m_pB2SData->GetLEDAreas())
   {
      ...
      {
         pLedArea->SetRect(
            { (int)(pLedArea->GetRect().x / rescaleX), (int)(pLedArea->GetRect().y / rescaleY), (int)(pLedArea->GetRect().w / rescaleX), (int)(pLedArea->GetRect().h / rescaleY) });
      }
   }
   ...
   {
      m_backglassCutOff = { (int)(roundf(m_backglassCutOff.x / rescaleX)), (int)(roundf(m_backglassCutOff.y / rescaleY)), (int)(roundf(m_backglassCutOff.w / rescaleX)),
         (int)(roundf(m_backglassCutOff.h / rescaleY)) };
   }
}
...
void B2SScreen::ScaleControl(B2SBaseBox* pControl, float rescaleX, float rescaleY, bool isOnDMD, bool flipY)
{
   if (!isOnDMD && !SDL_RectEmpty(&m_backglassCutOff))
   {
      if (SDL_HasRectIntersection(&m_backglassCutOff, &pControl->GetRect()))
         ...
         pControl->SetRectangleF(
            { pControl->GetLeft() / rescaleX, (pControl->GetTop() - m_backglassSmallGrillHeight) / rescaleY, pControl->GetWidth() / rescaleX, pControl->GetHeight() / rescaleY });
      else
...
/* Texture* B2SScreen::CutOutImage(Texture* pSourceImage, int grillheight, int smallgrillheight)
{
    Texture* pImageWithoutSmallGrill = SDL_CreateSurface(pSourceImage->w, pSourceImage->h - smallgrillheight, pSourceImage->format);
    Texture* pImageBackglass = PartFromImage(pSourceImage, { 0, 0, pSourceImage->w, pSourceImage->h - grillheight - smallgrillheight });
    Texture* pImageGrill = PartFromImage(pSourceImage, { 0, pSourceImage->h - grillheight, pSourceImage->w, grillheight });
...
Texture* B2SScreen::PartFromImage(Texture* pSourceImage, SDL_Rect rect)
{
   Texture* pImagePart = SDL_CreateSurface(rect.w, rect.h, pSourceImage->format);
   if (!pImagePart)
      return NULL;
...
Texture* B2SScreen::ResizeImage(Texture* pSourceImage, int grillheight)
{
   Texture* pImageWithoutGrill = SDL_CreateSurface(pSourceImage->w, pSourceImage->h - grillheight, pSourceImage->format);
...
Texture* B2SScreen::FlipImage(Texture* pSourceImage)
{
   Texture* pFlippedImage = SDL_CreateSurface(pSourceImage->w, pSourceImage->h, pSourceImage->format);
   if (!pFlippedImage)
      return NULL;
