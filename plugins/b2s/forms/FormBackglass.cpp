...
void FormBackglass::Render()
{
   ...
   if (IsVisible() && GetBackgroundImage()) {
      ...
      // FIXME pGraphics->DrawImage(GetBackgroundImage(), NULL, NULL);
...
void FormBackglass::DrawImage(B2SPictureBox* pPicbox)
{
   if (pPicbox) {
   ...
   try {
      ...
      if (topnode->FirstChildElement("Illumination")) {
         for (auto innerNode = topnode->FirstChildElement("Illumination")->FirstChildElement("Bulb");  innerNode != nullptr; innerNode = innerNode->NextSiblingElement("Bulb")) {
            ...
            Texture* pImage = Base64ToImage(innerNode->Attribute("Image"));
            Texture* pOffImage = NULL;
            ...
            if (picboxrotatesteps > 0) {
               if (picboxtype == ePictureBoxType_SelfRotatingImage && m_pB2SData->GetRotatingImages()->size() == 0) {
                  ...
                  // FIXME RotateImage(pPicbox, picboxrotatesteps, picboxrotationdirection, ePictureBoxType_SelfRotatingImage);
               }
               else if (picboxtype == ePictureBoxType_MechRotatingImage && m_pB2SData->GetRotatingImages()->size() == 0) {
                  // FIXME RotateImage(pPicbox, picboxrotatesteps, picboxrotationdirection, ePictureBoxType_MechRotatingImage, romidtype, romid);
               }
            }
         }
      }
            ...
            for (auto innerNode = topnode->FirstChildElement("Reels")->FirstChildElement("Images")->FirstChildElement("Image");  innerNode != nullptr; innerNode = innerNode->NextSiblingElement("Image")) {
               ...
               if (innerNode->FindAttribute("CountOfIntermediates")) {
                  ...
                  for (int i = 1; i <= countOfIntermediates; i++) {
                     ...
                     Texture* intimage = Base64ToImage(innerNode->Attribute(("IntermediateImage" + std::to_string(i)).c_str()));
...
void FormBackglass::ResizeSomeImages()
{
   ...
   if (m_pDarkImage4Authentic) {
      ...
      Texture* pImage = ResizeSurface(m_pDarkImage4Authentic, m_pB2SScreen->GetBackglassSize().w, m_pB2SScreen->GetBackglassSize().h);
   ...
   if (m_pTopLightImage4Authentic) {
      Texture* pImage = ResizeSurface(m_pTopLightImage4Authentic, m_pB2SScreen->GetBackglassSize().w, m_pB2SScreen->GetBackglassSize().h);
      SDL_DestroySurface(m_pTopLightImage4Authentic);
      m_pTopLightImage4Authentic = pImage;
   }
   if (m_pTopLightImage4Fantasy) {
      Texture* pImage = ResizeSurface(m_pTopLightImage4Fantasy, m_pB2SScreen->GetBackglassSize().w, m_pB2SScreen->GetBackglassSize().h);
      SDL_DestroySurface(m_pTopLightImage4Fantasy);
      m_pTopLightImage4Fantasy = pImage;
   }
   if (m_pSecondLightImage4Authentic) {
      Texture* pImage = ResizeSurface(m_pSecondLightImage4Authentic, m_pB2SScreen->GetBackglassSize().w, m_pB2SScreen->GetBackglassSize().h);
      SDL_DestroySurface(m_pSecondLightImage4Authentic);
      m_pSecondLightImage4Authentic = pImage;
   }
   if (m_pSecondLightImage4Fantasy) {
      Texture* pImage = ResizeSurface(m_pSecondLightImage4Fantasy, m_pB2SScreen->GetBackglassSize().w, m_pB2SScreen->GetBackglassSize().h);
      SDL_DestroySurface(m_pSecondLightImage4Fantasy);
      m_pSecondLightImage4Fantasy = pImage;
   }
   if (m_pTopAndSecondLightImage4Authentic) {
      Texture* pImage = ResizeSurface(m_pTopAndSecondLightImage4Authentic, m_pB2SScreen->GetBackglassSize().w, m_pB2SScreen->GetBackglassSize().h);
   ...
   if (m_pTopAndSecondLightImage4Fantasy) {
      Texture* pImage = ResizeSurface(m_pTopAndSecondLightImage4Fantasy, m_pB2SScreen->GetBackglassSize().w, m_pB2SScreen->GetBackglassSize().h);
      ...
      for(const auto& [key, pPicbox] : *m_pB2SData->GetIlluminations()) {
         if (pPicbox->GetPictureBoxType() == ePictureBoxType_StandardImage) {
            if (pPicbox->GetBackgroundImage()) {
               ...
               Texture* pImage = ResizeSurface(pPicbox->GetBackgroundImage(), rect.w, rect.h);
               SDL_DestroySurface(pPicbox->GetBackgroundImage());
               pPicbox->SetBackgroundImage(pImage);
               if (pPicbox->GetOffImage()) {
                  Texture* pOffImage = ResizeSurface(pPicbox->GetOffImage(), rect.w, rect.h);
...
void FormBackglass::RotateImage(B2SPictureBox* pPicbox, int rotationsteps, eSnippitRotationDirection rotationdirection, ePictureBoxType type, eRomIDType& romidtype, int& romid)
{
   if (pPicbox && rotationsteps > 0) {
      ...
      while (rotatingAngle < 360) {
         Texture* pImage = RotateSurface(pPicbox->GetBackgroundImage(), rotationdirection == eSnippitRotationDirection_AntiClockwise ? rotatingAngle : 360 - rotatingAngle);
...
/*Texture* FormBackglass::CreateLightImage(Texture* image, eDualMode dualmode, const string& firstromkey_)
{
   return CreateLightImage(image, dualmode, firstromkey_, string());
}

Texture* FormBackglass::CreateLightImage(Texture* image, eDualMode dualmode, const string& firstromkey_, const string& secondromkey_)
{
   int romid = 0;
   eRomIDType romidtype = eRomIDType_NotDefined;
   bool rominverted = false;
...
Texture* FormBackglass::CreateLightImage(Texture* image, eDualMode dualmode, const string& firstromkey_, const string& secondromkey_, int& romid, eRomIDType& romidtype, bool& rominverted)
{
   ...
   // create image copy
   Texture* pImage = ResizeSurface(image, image->w, image->h);
...
/*Texture* FormBackglass::RotateSurface(Texture* source, int angle)
{
   SDL_LockSurface(source);

   Texture* const destination = SDL_CreateSurface(source->w, source->h, source->format);
...
Texture* FormBackglass::ResizeSurface(Texture* original, int newWidth, int newHeight)
{
    Texture* newSurface = SDL_CreateSurface(newWidth, newHeight, original->format);
    if (!newSurface)
       return NULL;
...
Texture* FormBackglass::Base64ToImage(const string& image)
{
   vector<unsigned char> imageData = base64_decode(image);
   Texture* pImage = new Texture();
   if (!pImage->LoadFromMemory(imageData.data(), imageData.size()))
      return nullptr;
   return pImage;
}
